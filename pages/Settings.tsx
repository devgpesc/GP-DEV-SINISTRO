    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
            ...
            disabled={saving}
            className={`px-8 py-3 rounded-2xl font-black flex items-center gap-2 transition-all shadow-xl uppercase text-xs tracking-widest w-full md:w-auto justify-center ${saved ? 'bg-green-600 text-white' : 'bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-70'}`}
        >
              ...
              <div className="flex overflow-x-auto md:block md:space-y-1 gap-2 pb-2 md:pb-0 custom-scrollbar">
                  {tabs.map(tab => (
                      <button
                        ...
                        className={`flex-shrink-0 md:w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:bg-slate-100'}`}
                      >
                          <tab.icon size={18}/>
                          {tab.label}
                      </button>
                  ...
                  <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                      ...
                      {loadingUsers ? (
                         ...
                         <div className="space-y-3">
                            {usersList.map(user => (
                                <div key={user.id} className="p-4 rounded-2xl border border-slate-100 hover:border-blue-200 transition-all flex flex-col md:flex-row md:items-center justify-between group gap-4 md:gap-0">
