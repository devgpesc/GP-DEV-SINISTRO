...
import { 
  ...
  LayoutDashboard, Zap, Globe, AlertCircle, CheckCircle 
} from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message === 'Invalid login credentials' ? 'Credenciais inválidas.' : 'Erro ao conectar.');
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // O redirecionamento é tratado pelo Supabase + AuthContext
    } catch (err: any) {
      setError('Erro ao iniciar Google Auth.');
      setLocalLoading(false);
    }
  };
  ...
  // Previne flash de conteúdo enquanto verifica sessão
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col justify-between p-16 overflow-hidden bg-[#0A1628]">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-lg max-w-md leading-relaxed font-light">
            Plataforma SaaS integrada para controle total de processos, auditoria financeira e compliance em tempo real.
          </p>
        </div>
        ...
        <div className="relative z-10 space-y-4">
           {[
             ...
             { icon: Globe, title: 'Acesso Global', desc: 'Gestão multi-tenant segura e escalável.' }
           ].map((item, idx) => (
        ...
        <div className="w-full max-w-[420px] bg-white p-8 md:p-10 rounded-[24px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-slate-100 animate-in slide-in-from-right-8 duration-700">
          ...
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">
              {isResetting ? 'Recuperar Acesso' : 'Bem-vindo de volta'}
            </h2>
            <p className="text-slate-500 text-sm mt-2 font-medium">
              {isResetting ? 'Enviaremos um link para seu e-mail.' : 'Acesse sua conta para continuar.'}
            </p>
          </div>
          ...
          {successMsg && (
             <div className="mb-6 p-4 bg-green-50 border border-green-100 rounded-xl flex items-start gap-3 text-green-700 animate-in fade-in slide-in-from-top-2">
          ...
          {!isResetting ? (
            <div className="space-y-6">
              ...
              <div className="relative flex items-center justify-center py-2">
                ...
                <span className="relative bg-white px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">OU EMAIL</span>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                 ...
                 <div className="flex justify-end">
                    <button type="button" onClick={() => setIsResetting(true)} className="text-xs font-bold text-blue-600 hover:text-blue-700 hover:underline">
                      Esqueceu a senha?
                    </button>
                 </div>
            ...
            <div className="space-y-6">
              <form onSubmit={handleResetPassword} className="space-y-5">
                 ...
                 <button 
                    type="submit" 
                    disabled={localLoading}
                    className="w-full py-3.5 bg-slate-800 text-white rounded-xl font-bold text-sm shadow-lg hover:bg-slate-900 transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                 >
                    {localLoading ? <Loader2 className="animate-spin" size={20}/> : 'Enviar Link de Recuperação'}
                 </button>
