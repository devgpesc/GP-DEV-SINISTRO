...
const writeCookie = (name: string, value: string, maxAge = PKCE_COOKIE_MAX_AGE) => {
  if (!isBrowser()) return;
  // Evita cookies gigantes (sessao JWT), que browsers descartam ou enviam parcialmente.
...
/** Remove cookies de auth legados ou grandes que estouram o header Cookie. */
export const purgeOversizedAuthCookies = () => {
  ...
  try {
    ...
    /* Cookie access can be blocked by the browser. */
  }
};
...
export const authStorage = {
  getItem(key: string): string | null {
    ...
    return isPkceKey(key) ? readCookie(key) : null;
  },
