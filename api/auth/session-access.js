...
export default async function handler(req, res) {
  ...
  try {
    ...
    const related = email ? await findUsersByEmail(admin, email) : [];
    if (!related.some((u) => u.id === user.id)) related.push(user);

    // Coleta memberships de todas as contas do e-mail e propaga para a sessao atual.
    ...
    // Tambem pega convite pending/accepted do e-mail.
    if (email) {
      ...
      for (const invite of invites || []) {
        if (invite.tenant_id && invite.status !== 'cancelled') {
    ...
    for (const [tenant_id, role] of membershipMap.entries()) {
      for (const relatedUser of related) {
        await admin.from('organization_members').upsert(
          { tenant_id, user_id: relatedUser.id, role },
    ...
    await admin.from('profiles').upsert(
      {
        id: user.id,
        email: email || null,
        full_name: user.user_metadata?.full_name || user.user_metadata?.name || (email ? email.split('@')[0] : 'Usuario'),
    ...
    return sendJson(res, 200, {
      ...
      userId: user.id,
      email,
      relatedUserIds: related.map((u) => u.id),
      membershipCount: memberships?.length || 0,
      memberships: memberships || [],
      tenants,
      repaired: membershipMap.size > 0,
    });
  } catch (error) {
